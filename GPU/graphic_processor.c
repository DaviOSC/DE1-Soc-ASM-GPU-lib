...
createMappingMemory() {
    ...
    h2p_lw_dataA_addr = virtual_base
                        + ((unsigned long)(ALT_LWFPGASLVS_OFST + DATA_A_BASE) & (unsigned long)(HW_REGS_MASK));
    h2p_lw_dataB_addr = virtual_base
                        + ((unsigned long)(ALT_LWFPGASLVS_OFST + DATA_B_BASE) & (unsigned long)(HW_REGS_MASK));
    h2p_lw_wrReg_addr = virtual_base
                        + ((unsigned long)(ALT_LWFPGASLVS_OFST + WRREG_BASE) & (unsigned long)(HW_REGS_MASK));
    h2p_lw_wrFull_addr = virtual_base
                         + ((unsigned long)(ALT_LWFPGASLVS_OFST + WRFULL_BASE) & (unsigned long)(HW_REGS_MASK));
    h2p_lw_screen_addr = virtual_base
                         + ((unsigned long)(ALT_LWFPGASLVS_OFST + SCREEN_BASE) & (unsigned long)(HW_REGS_MASK));
    h2p_lw_result_pulseCounter_addr =
        virtual_base + ((unsigned long)(ALT_LWFPGASLVS_OFST + RESET_PULSECOUNTER_BASE) & (unsigned long)(HW_REGS_MASK));
    return 1;
}
...
// dataA -> r0, dataB -> r1
void
sendInstruction(unsigned long dataA, unsigned long dataB) {
    if (isFull() == 0) {
...
//opcode -> r0, reg -> r1, memory_address -> r2

/* ================================================================================
Function to defines the dataA bus of the instruction.
===================================================================================*/
static unsigned long
dataA_builder(int opcode, int reg, int memory_address) {
...
void
setPolygon(int address, int opcode, int color, int form, int mult, int ref_point_x, int ref_point_y) {
...
/*registrador -> r0, x -> r1, y -> r2, offset -> r3, activation_bit -> stack*/
void
...
/*R -> r0, G -> r1, B -> r2*/
void
set_background_color(int R, int G, int B) {
...
/*column, -> r0 line, -> r1 R, -> r2 G, -> r3 B -> stack */
void
set_background_block(int column, int line, int R, int G, int B) {
...
increase_coordinate(Sprite* sp, int mirror) {
    switch ((*sp).direction) {
        case LEFT: // 0 degrees
            ...
            if (mirror == 1) {
                if ((*sp).coord_x < 1) { // Mirrors the sprite's position when it reaches
        ...
        case UPPER_RIGHT: // 45 degrees
            ...
            if (mirror == 1) {
                if ((*sp).coord_y < 0) { // Mirrors the sprite's position when it reaches
                                         // the upper limit of the screen.
                    (*sp).coord_y = 480;
                } else if ((*sp).coord_x > 640) { // Mirrors the sprite's position when it
        ...
        case UPPER_LEFT: // 135 degrees
            ...
            if (mirror == 1) {
                if ((*sp).coord_y < 0) { // Mirrors the sprite's position when it reaches
                                         // the upper limit of the screen.
                    (*sp).coord_y = 480;
                } else if ((*sp).coord_x < 1) { // Mirrors the sprite's position when it
        ...
        case RIGHT:                        // 180 degrees
            ...
            if (mirror == 1) {
                if ((*sp).coord_x > 640) { // Mirrors the sprite's position when it
                                           // reaches the right edge of the screen.
                    (*sp).coord_x = 0;
                }
            } else {
        ...
        case BOTTOM_LEFT: // 225 degrees
            ...
            (*sp).coord_y += (*sp).step_y;
            if (mirror == 1) {
                if ((*sp).coord_y > 480) { // Mirrors the sprite's position when it
                                           // reaches the lower limit of the screen.
                    (*sp).coord_y = 0;
                } else if ((*sp).coord_x < 1) { // Mirrors the sprite's position when it
                                                // reaches the left edge of the screen.
                    (*sp).coord_x = 640;
